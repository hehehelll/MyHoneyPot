...
class KippoGraph
{
    ...
    public function createTop10Passwords()
    {
        ...
        if (count($rows)) {
            $chart = new VerticalBarChart(600, 300);// 参数表示需要创建的图像的宽和高
            $dataSet = new XYDataSet();// 实例化一个 XY轴数据对象

            foreach ($rows as $row) {
                $dataSet->addPoint(new Point($row['password'], $row['COUNT(password)']));#增加数据集
            }


            $chart->setDataSet($dataSet);// 把这个数据集合传递给图形对象
            $chart->setTitle(TOP_10_PASSWORDS);// 设置图形的标题
            $chart->render(DIR_ROOT . "/generated-graphs/top10_passwords.png");//生成图形
        }
    }
    ...
    public function createTop10Combinations()
    {
        ...
        if (count($rows)) {
            $chart = new VerticalBarChart(600, 300);//柱状图
            $pie_chart = new PieChart(600, 300);//饼状图
            $dataSet = new XYDataSet();
    ...
    public function createSuccessesPerDay()
    {
        ...
        if (count($rows)) {
            ...
            $chart = new LineChart(600, 300);//折线
            $dataSet = new XYDataSet();
